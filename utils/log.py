...
class Logger(object):
    def __init__(self, logger_name='framework'):
        ...
        c = Config().get('loginfo')
        self.log_file_name = c.get('file_name') if c and c.get('file_name') else 'info.log'  # 日志文件
...
class Logbug(object):
    def __init__(self, logger_name='framework1'):
        self.logger = logging.getLogger(logger_name)
        logging.root.setLevel(logging.NOTSET)
        c = Config().get('logbug')
        self.log_file_name = c.get('file_name') if c and c.get('file_name') else 'bug.log'  # 日志文件
        self.backup_count = c.get('backup') if c and c.get('backup') else 5  # 保留的日志数量
        # 日志输出级别
        self.console_output_level = c.get('console_level') if c and c.get('console_level') else 'WARNING'
        self.file_output_level = c.get('file_level') if c and c.get('file_level') else 'DEBUG'
